...
```http
POST /build_taxonomy?similarity_threshold=<float>&max_tags=<int>
```
...
**Description**: Builds a cross-workspace hierarchical taxonomy by clustering all items' semantic embeddings. The taxonomy is self-emerging — themes and sub-themes are derived from the actual content of all items across all workspaces using two-level agglomerative clustering on the raw 3072-dimensional embeddings with cosine distance. Cluster names are generated by GPT-4.1 in four languages (English, Dutch, Hebrew, Arabic).

Each item receives multi-label topic tags stored in `metadata.topics`. The full taxonomy reference is saved to the `__global__/.taxonomy` Firestore document and can be retrieved via `GET /taxonomy`.

**Stream Events**:
```
data: [timestamp, {"msg": "Loading items from workspace-1..."}]

data: [timestamp, {"msg": "Clustering 500 items into hierarchical taxonomy..."}]

data: [timestamp, {"msg": "Theme 0: Climate & Disaster (85 items)"}]

data: [timestamp, {"msg": "Saving topics to Firestore..."}]
...
- Creates/overwrites the `__global__/.taxonomy` document (see [Taxonomy Document](#taxonomy-document))
...
```json
{
  ...
  "metadata": {
    ...
    // Taxonomy Tags (set by build_taxonomy endpoint)
...
Stored in Firestore at `__global__/.taxonomy`. Generated by the `build_taxonomy` endpoint and served by `GET /taxonomy`.
...
4. **`__global__`** - Cross-workspace data
